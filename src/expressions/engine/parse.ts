...
export type Expr =
  ...
  // SEGMENTOS, e não uma string com pontos. `{["a.b"]}` é UM segmento que
  // contém ponto — uma string não consegue distinguir isso de caminhar a->b,
  // e essa ambiguidade é exatamente o que os brackets existem pra resolver.
  // A forma nua (`{a.b}`) é dividida no ponto aqui, então ela chega igual.
...
class Parser {
  ...
  private parseAtom(depth: number): Expr {
    ...
    switch (token.kind) {
      ...
      // Path delimitado: o lexer já resolveu os segmentos (inclusive os que
      // têm ponto ou espaço dentro), então aqui não há nada pra dividir. É a
      // diferença que justifica o token separado do `ident`.
      case "path":
