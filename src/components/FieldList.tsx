...
export function FieldList({ schemas, selectedIds, onSelect, onRemove, onToggleLock }: Props) {
  if (schemas.length === 0) {
    return <p className="text-xs text-slate-400">Nenhum campo ainda — use os botões "+" acima do canvas.</p>;
  }
      ...
      {schemas.map((schema) => (
        <li
          ...
          className={`flex items-center gap-1 rounded-lg border px-1.5 py-1 transition-colors ${
            selectedIds.includes(schema.id) ? "border-sky-500 bg-sky-50" : "border-slate-200 hover:bg-slate-50"
          }`}
        >
          <button
            type="button"
          ...
          >
            ...
            <span className="ml-1.5 text-slate-400">{typeLabel[schema.type]}</span>
          </button>
          <Button
            variant="ghost"
            size="icon"
