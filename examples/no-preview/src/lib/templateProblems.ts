...
// Nome dado pela pessoa (`page.name`) é DADO e sai como está; só o rótulo
// posicional de fallback é interface, e vem do dicionário da casca — o mesmo
// texto que a aba da página mostra (components/PageTabs.tsx).
function pageLabel(page: TemplatePage, index: number, locale: Locale): string {
  return page.name?.trim() || t(locale).pages.label(index + 1);
}
