...
type Props = {
  ...
  // One value, two destinations: it goes into the `<Designer locale>` (the
  // editor speaks that language) and into the shell's `t(locale)` (the field
  // explorer's modal, below, speaks the same one).
  locale: Locale;
};
...
// The <Designer> PRESET, not the parts — it is this example's identity on
// the spectrum (see its README.md). Everything the field explorer needs from
// the editor fits in two public props of the preset: `onCanvasDrop` (dropping
// a field on the page) and `dataSources` (known arrays become a dropdown in
// the binding editor, instead of a hand-typed path).
//
// What does NOT fit is reading the editor's selection from outside — that
// requires assembling the <DesignerProvider> by hand, and it is precisely what
// report-builder does with its SelectedFieldBar. Here, preset by definition.
