...
// FieldTree.tsx).
//
// A function (and not a module constant) because the LABEL is UI text and
// follows the shell's language. The `path` is the only side that does not
// change: `pageNumber` is the token the PDF engine resolves, so it is data —
// translating it would break the template.
