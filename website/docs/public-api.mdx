...
// Canvas zoom - its OWN context, so only what reads it re-renders
useDesignerZoom()                          // { zoom, min, max, step, setZoom, zoomIn, zoomOut, reset, fitWidth, fitHeight, viewportRef }
clampZoom(n), ZOOM_MIN, ZOOM_MAX, ZOOM_STEP // the limits the canvas uses; clampZoom turns NaN into 1
type DesignerZoomValue
// <DesignerCanvas hideZoombar /> removes the default bar - see Composing the Designer
