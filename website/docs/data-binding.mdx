...
an argument (`{CURRENCY(SUM(rows.total), "$")}`), with one exception:
**two function calls combined by an operator in the same expression**
(`{SUM(a) - SUM(b)}`) doesn't resolve correctly — pre-compute the value
in the JSON or split it into two tokens instead.
