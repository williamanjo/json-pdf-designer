...
type Template = {
  version?: TemplateVersion; // versão do formato — ausente = 1, ver "Versionamento de template"
...
`Template.backgroundImage` coloca uma imagem (um PNG data URI) atrás de
tudo, tanto no editor quanto no PDF final. O upload do `<Designer>` aceita
PNG/JPEG; um papel timbrado que só existe em PDF precisa ser exportado pra
PNG antes.
...
Descomprimir `.woff2` precisa do `wawoff2`, uma **dependência peer
opcional** — não vem instalada sozinha (a maioria dos projetos nunca
embute fonte customizada, e é um binário WASM grande). Rode `npm
install wawoff2` só se você realmente for passar um arquivo `.woff2`;
sem ele instalado, passar um `.woff2` lança um erro claro pedindo pra
instalar ou converter a fonte pra `.ttf`/`.otf` offline antes.
