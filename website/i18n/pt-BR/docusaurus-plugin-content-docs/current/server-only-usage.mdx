...
importa `/server`. Marcar os nossos próprios peers react como opcionais não
bastava sozinho; o `react-rnd` também precisou virar opcional (virou, na
2.0.0). Um install de backend hoje resolve `fontkit`, `pdf-lib` e
...
usando só esse subpath mais o `PdfPreview` de
`json-pdf-designer/preview`, sem nenhum `<Designer>` à vista.
