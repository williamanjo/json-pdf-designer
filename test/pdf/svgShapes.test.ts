...
describe("roundedRectPath", () => {
  ...
  it("produces the exact expected path for width=10, height=10, radius=3", () => {
    // Worked out by hand from the algorithm: half = min(10,10)/2 = 5;
  ...
  it("supports independent per-corner values", () => {
    ...
    // tl=1, tr=2, bl=3, br=4 (all <= 5, no clamp).
  ...
  it("clamps a radius larger than half of the smaller dimension", () => {
    // width=10, height=6 -> half = min(10,6)/2 = 3. radius=8 -> clamped to 3,
    // the same on every corner (through the single number).
  ...
  it("clamps each corner independently in the object form", () => {
    // width=10, height=6 -> half = 3. tl=8 (clamps to 3), tr=1 (no clamp).
